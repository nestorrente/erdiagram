import BetaEntityRelationshipModelParser from '@/erdiagram/parser/beta/BetaEntityRelationshipModelParser';
import {
	Cardinality,
	Direction,
...
const entityRelationshipModelParser = new BetaEntityRelationshipModelParser();
			...
			entities: [
				{
					...
					properties: [
						...
						createSimpleProperty('k', EntityPropertyType.BLOB),
					]
				}
			],
			relationships: []
...
A <->!? E eAlias (RelationshipName)
	...
	test('Allow unknown entities', () => {
		...
		const model = new BetaEntityRelationshipModelParser({
