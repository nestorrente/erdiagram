...
export default class OLDJavaClassModelToCodeConverter implements ClassModelToCodeConverter {
	...
	private addValidationAnnotationsToFieldIfApply(field: ClassFieldDescriptor, fieldLines: string[], usedTypes: JavaType[]) {
		if (this.config.useValidationAnnotations) {
			const annotations = this.validationAnnotationsGenerator.getAnnotations(field);
